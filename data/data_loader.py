...
def get_dataset(batch_size=64):
    ...
    transform = transform_dataset()
    # Download and load the training data
    train_raw_data = datasets.CIFAR10(
        root="data/images/transformed_data/train_data", train=True, download=True, transform=transform)
    test_raw_data = datasets.CIFAR10(
        root="data/images/transformed_data/test_data", train=False, download=True, transform=transform)
    ...
    train_data_loader = torch.utils.data.DataLoader(
        train_raw_data, batch_size=batch_size)
    test_data_loader = torch.utils.data.DataLoader(
        test_raw_data, batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    train_image_label_dictionary = build_label_image_dictionary(
        train_data_loader, 500)
